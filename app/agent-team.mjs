...
import {HELPER_HOMES as slots,HelperJourney,helperStation,helperTarget} from './helper-navigation.mjs';
export function createAgentTeam(scene,host){
 ...
 function reconcile(){
  ...
  shown.forEach((a,i)=>{if(members.has(a.id))return;const root=new T.Group(),body=clone(library.scene),materials=[],color=AGENT_COLORS[i];
   ...
   const mixer=new T.AnimationMixer(body),actions=Object.fromEntries(library.animations.map(c=>[c.name,mixer.clipAction(c)]));members.set(a.id,{root,body,ring,materials,label,mixer,actions,journey:new HelperJourney(slots[i]),clip:null,elapsed:i*1.3,phase:i*.37,status:'Ready'});
 ...
 function update(dt,camera){const lang=document.documentElement.lang,reserved=[];for(const a of visibleAgents(input)){const m=members.get(a.id);if(!m)continue;m.elapsed+=dt;const status=agentStatus(a,connected),station=helperStation(a,status),journey=m.journey;
  if(station){const target=helperTarget(station,reserved);if(target)reserved.push(target);journey.go(station,target);}
  else if(['Finished','Interrupted','Ready'].includes(status))journey.go(null,journey.home);
  else journey.pause();
  journey.update(dt);m.root.position.set(journey.position[0],.12,journey.position[1]);
  const turn=T.MathUtils.euclideanModulo(journey.heading-m.root.rotation.y+Math.PI,2*Math.PI)-Math.PI;m.root.rotation.y+=turn*(1-Math.exp(-dt*10));
  const arrived=journey.state==='working',requested=journey.state==='walking'?'Walk':arrived?agentClip(status,station,m.elapsed):'Idle',clip=m.actions[requested]?requested:'Idle';m.status=status;
  ...
  const p=m.root.position.clone().add(new T.Vector3(0,1.52,0)).project(camera);m.label.hidden=p.z>1||p.z< -1;m.label.style.left=((p.x*.5+.5)*host.clientWidth)+'px';m.label.style.top=((-p.y*.5+.5)*host.clientHeight)+'px';m.label.textContent=translate(a.title,lang)+' · '+(journey.state==='walking'?(lang==='th'?'กำลังเดิน':'Walking'):translate(status,lang))+(station?' · '+(lang==='th'?'สถานี ':'Station ')+station.split('-')[1]:'');
 }}
 ...
 return {setAgents,update,getState:()=>[...members].map(([id,m])=>({id,status:m.status,animation:m.clip,animationTime:m.actions[m.clip]?.time||0,animationWeight:m.actions[m.clip]?.getEffectiveWeight()||0,stationId:m.journey.stationId,movement:m.journey.state,position:[...m.journey.position],target:m.journey.target,heading:m.root.rotation.y})),dispose(){disposed=true;for(const id of [...members.keys()])remove(id);releaseLibrary();}};
}
