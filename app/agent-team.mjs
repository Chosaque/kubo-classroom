...
import {AGENT_COLORS,agentStatus,agentClip} from './agent-team-state.mjs';
...
export function createAgentTeam(scene,host){
 ...
 function reconcile(){
  if(!library||disposed)return;const shown=input.slice(0,4);for(const id of members.keys())if(!shown.some(a=>a.id===id))remove(id);
  shown.forEach((a,i)=>{if(members.has(a.id)){const m=members.get(a.id);m.root.position.set(slots[i][0],.12,slots[i][1]);m.ring.material.color.set(AGENT_COLORS[i]);m.materials.filter(x=>x.name.includes('Agent Jacket')).forEach(x=>x.color.set(AGENT_COLORS[i]));m.label.style.setProperty('--agent-color',AGENT_COLORS[i]);return;}const root=new T.Group(),body=clone(library.scene),materials=[],color=AGENT_COLORS[i];
   ...
   const mixer=new T.AnimationMixer(body),actions=Object.fromEntries(library.animations.map(c=>[c.name,mixer.clipAction(c)]));members.set(a.id,{root,body,ring,materials,label,mixer,actions,clip:null});
 ...
 function update(dt,camera){const lang=document.documentElement.lang;for(const a of input.slice(0,4)){const m=members.get(a.id);if(!m)continue;const status=agentStatus(a,connected),clip=agentClip(status,a.stationId);
  if(clip!==m.clip){const next=m.actions[clip]||m.actions.Idle;if(next){next.reset().play();if(m.clip)m.actions[m.clip]?.crossFadeTo(next,.45,false);}m.clip=clip;}
  m.mixer.update(dt);m.body.traverse(o=>{if(o.name.startsWith('PROP_File'))o.visible=status==='Working';});
 ...
 return {setAgents,update,dispose(){disposed=true;for(const id of [...members.keys()])remove(id);releaseLibrary();}};
}
